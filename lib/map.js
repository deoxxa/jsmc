...
//            chunk.set_block_type(x, z, y, 20);
          ...
          if (Math.random() > 0.9995 && chunk.get_block_type(x, z, y) === 0) {
...
Map.prototype.calculate_light = function calculate_light(chunk_x, chunk_z) {
  var key = this.make_key(chunk_x, chunk_z);
  ...
  for (var y = 0; y < 256; ++y) {
    for (var i = 0; i < 16; ++i) {
      spread_sky.push(  [((chunk_x << 4) + i),      ((chunk_z + 1) << 4),      y]);
      spread_sky.push(  [((chunk_x << 4) + i),      ((chunk_z - 1) << 4) + 15, y]);
      spread_sky.push(  [((chunk_x + 1) << 4),      ((chunk_z << 4) + i),      y]);
      spread_sky.push(  [((chunk_x - 1) << 4) + 15, ((chunk_z << 4) + i),      y]);
      spread_block.push([((chunk_x << 4) + i),      ((chunk_z + 1) << 4),      y]);
      spread_block.push([((chunk_x << 4) + i),      ((chunk_z - 1) << 4) + 15, y]);
      spread_block.push([((chunk_x + 1) << 4),      ((chunk_z << 4) + i),      y]);
      spread_block.push([((chunk_x - 1) << 4) + 15, ((chunk_z << 4) + i),      y]);
    }
  }

  while (spread_sky.length) {
    var job = spread_sky.shift();
    this.spread_light_sky(job[0], job[1], job[2], spread_sky, chunk_x, chunk_z, chunk);
  }

  while (spread_block.length) {
    var job = spread_block.shift();
    this.spread_light_block(job[0], job[1], job[2], spread_block, chunk_x, chunk_z, chunk);
  }
};
