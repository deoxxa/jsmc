...
Serialiser.prototype.write = function write(packet) {
  switch (packet.pid) {
    ...
    case 0x17: {
      this.emit("data", S().uint8(packet.pid).uint32be(packet.eid).uint8(packet.type).uint(packet.x).uint(packet.y).uint(packet.z).uint8(packet.yaw).uint8(packet.pitch).uint(packet.data).uint16be(packet.speed_x).uint16be(packet.speed_y).uint16be(packet.speed_z).result());
      break;
    }
