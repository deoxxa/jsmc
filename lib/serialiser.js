...
S.prototype.mcintdegree = function mcintdegree(data) {
  var b = Math.round(data % 360);

  while (b < 1) {
    b += 360;
  }

  return this.uint8(Math.round(b / 360 * 255));
};
...
Serialiser.prototype.write = function write(packet) {
  switch (packet.pid) {
    ...
    case 0x14: {
      this.emit("data", S().uint8(packet.pid).uint32be(packet.eid).mcstring16(packet.name).mcabsint(packet.x).mcabsint(packet.y).mcabsint(packet.z).mcintdegree(packet.yaw).mcintdegree(packet.pitch).uint16be(packet.current_item).mcmetadata(packet.metadata).result());
      break;
    }
