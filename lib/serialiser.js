...
Serialiser.prototype.write = function write(packet) {
  switch (packet.pid) {
    ...
    case 0x09: {
      this.emit("data", S().uint8(packet.pid).int32be(packet.dimension).int8(packet.difficulty).int8(packet.gamemode).int16be(packet.world_height).mcstring16(packet.level_type).result());
      break;
    }
